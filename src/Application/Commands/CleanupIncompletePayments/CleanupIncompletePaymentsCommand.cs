...
namespace Application.Commands
{
    ...
    public class CleanupIncompletePaymentsCommandHandler : IRequestHandler<CleanupIncompletePaymentsCommand, CleanupIncompletePaymentsCommandResult>
    {
        ...
        private GetPaymentResult _getPaymentResult;
        ...
        {
            var threshold = DateTime.Now.AddMinutes(-_thresholdInMiutes);

            _incompletePayments = (await _paymentRepository.List(
                x => x.PaymentId != null
                && x.Finished == false 
                && x.CreatedDate <= threshold)).Data;
        ...
        {
            _getPaymentResult = await _govUKPayApiClient.GetAPaymentAsync(_incompletePayment.PaymentId);
        }

        private async Task UpdatePaymentStatus()
        {
            _incompletePayment.UpdateStatus(_getPaymentResult.State);
